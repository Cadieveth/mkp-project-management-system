...
export default function ProbabilityDropdown({
  onChange,
  value,
  helperText,
  errorText,
  ...
  const dataFetch = async () => {
    try {
      ...
      const response = await fetch(
        `${config.api_master}/probability/list?keyword=&status=Y&limit=25&offset=0`,
        {
          method: "GET",
          headers: headers,
        }
      ...
      if (response.ok) {
        ...
        const dataToPush = result.map((item) => ({
          value: item.id,
          // label: item.probabilityName + " (" + item.description + ")",
          label: item.probabilityName + "  -  " + item.description,
        }));
        setList(dataToPush);
